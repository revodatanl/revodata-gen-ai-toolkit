...
N_DOCUMENTS_TO_RETRIEVE_RERANKED = 8
...
class ConversationalRAG(ChatAgent):
    ...
    def _custom_convert_messages_to_dict(
        ...
        """Convert the messages to a different format"""
    ...
    def predict(
        ...
        assistant_toolcall = ChatAgentMessage(
            role="assistant",
            id=str(uuid.uuid4()),
            content=None,
            name="retrieve_docs",
            ...
            tool_calls=[
                ToolCall(
                    ...
                    function=Function(
                        name="retrieve",
                        arguments=latest_message,
                    ),
                )
            ],
        )

        assistant_toolcall_response = ChatAgentMessage(
            role="tool",
            id=str(uuid.uuid4()),
            content=str(reranked_context),
            name="retrieve_docs",
            tool_call_id=toolcall_id,
            tool_calls=None,
        )
    ...
    def predict_stream(
        ...
        # Yield the toolcall ('retrieval') and the documents retrieved.
        toolcall_id = str(uuid.uuid4())
